...
InitGraphics( )
{
	...
	Init05UniformBuffer( sizeof(Sporadic),   &MySporadicUniformBuffer );
	Fill05DataBuffer( MySporadicUniformBuffer,(void *) &Sporadic );

	Init05UniformBuffer( sizeof(Scene),      &MySceneUniformBuffer );
...
VK_PRESENT_MODE_SHARED_CONTINUOUS_REFRESH_KHR = 1000111001,
	...
	VkPresentModeKHR thePresentMode = (VkPresentModeKHR) 0;
	for( VkPresentModeKHR apm : acceptablePresentModes )
	{
		...
		if (thePresentMode != (VkPresentModeKHR) 0 )
			break;
	}
...
VK_PIPELINE_CREATE_DERIVATIVE_BIT
	...
	result = vkCreateGraphicsPipelines( LogicalDevice, (VkPipelineCache) 0, 1, IN &vgpci, PALLOCATOR, OUT pGraphicsPipeline );
...
Init14ComputePipeline( VkShaderModule computeShader, OUT VkPipeline * pComputePipeline  )
{
	...
	VkComputePipelineCreateInfo			vcpci[1];
		...
		vcpci[0].basePipelineHandle = (VkPipeline) 0;
		vcpci[0].basePipelineIndex = 0;

	result = vkCreateComputePipelines( LogicalDevice, (VkPipelineCache) 0, 1, &vcpci[0], PALLOCATOR, pComputePipeline );
...
DestroyAllVulkan( )
{
	...
	vkDestroyBuffer(LogicalDevice, MySporadicUniformBuffer.buffer, PALLOCATOR);
	vkFreeMemory(LogicalDevice, MySporadicUniformBuffer.vdm, PALLOCATOR);
	vkDestroyBuffer(LogicalDevice, MySceneUniformBuffer.buffer, PALLOCATOR);
	vkFreeMemory(LogicalDevice, MySceneUniformBuffer.vdm, PALLOCATOR);
	vkDestroyBuffer(LogicalDevice, MyObjectUniformBuffer.buffer, PALLOCATOR);
	vkFreeMemory(LogicalDevice, MyObjectUniformBuffer.vdm, PALLOCATOR);
...
RenderScene( )
{
	...
	vkAcquireNextImageKHR( LogicalDevice, IN SwapChain, IN UINT64_MAX,
				IN imageReadySemaphore, IN (VkFence) 0, OUT &nextImageIndex );
...
Reset( )
{
	...
	glm::vec3  look(0.,0.,0.);
	glm::vec3  up(0.,1.,0.);
	Scene.uSceneOrient      = glm::mat4( 1. );		// identity
	Scene.uView       = glm::lookAt( eye, look, up );
	Scene.uProjection = glm::perspective( FOV, (double)Width/(double)Height, 0.1, 1000. );
		Scene.uProjection[1][1] *= -1.;
	//Object.uNormal = glm::mat4(glm::inverseTranspose(glm::mat3(Matrices.uModelMatrix)));
